(ns citi_velocity.main
  "Kotodama WASM entrypoint for the Citi_velocity clean-room actor (L4) — Clojure port.
  ...
  facts (`citi_velocity.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "citi_velocity")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "citivelo_acc"
    :fields [:holderId :type :balance :currency :status]  :required [:type :balance]
    :coerce {:balance :float}          :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "citivelo_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"citi_velocity.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "citi_velocity-compat" :tier tier :entities entities} 200])
...
(defn start! [] :citi_velocity-compat/ready)
